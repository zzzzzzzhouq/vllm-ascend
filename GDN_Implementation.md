...
这一段解释 ③ 为什么存在、它在数学上做什么。用一个三个 token 的玩具例子从零走一遍。

**1. 理想情况**

块内 3 个 token 都在写同一个话题（标签相同），写入门为 1，无衰减。各自天真地想写：token 1 写 10，token 2 写 20，token 3 写 30。如果都直接写，记忆里这个话题会堆积 10+20+30 = 60，但它应该只保留最新的 30。

**2. 串行做法（原始递推，也是问题的由来）**

Delta 规则：写之前先查记忆，只写差额。

```
token 1：查到 0， 写 10−0  = 10，记忆 = 10
token 2：查到 10，写 20−10 = 10，记忆 = 20
token 3：查到 20，写 30−20 = 10，记忆 = 30 ✓
```

结果正确，但必须串行——token 2 的写入量取决于 token 1 写完后的记忆。

**3. 天真的并行为什么算错**

想并行，让每个 token 预先扣账：token 3 看到"前面天真地写了 10 和 20"，扣成 30−10−20 = 0。**错了**——token 2 实际写的不是 20，而是 20−10 = 10（它自己先修正过）。用别人的天真值扣账，而别人的真实写入已经被它自己的修正改掉了。修正之上还有修正，一层套一层——一笔笔顺序还清可以，但那就回到串行。

**4. 破局：把"还清所有债"写成方程组**

设三个 token 的真实写入是 u₁、u₂、u₃，把"该写多少"用方程写出来：
...
解出 u = (I+L)⁻¹·[10,20,30]ᵀ = [10,10,10]，验证 10+10+10 = 30 ✓。关键：解出来的三个数**互不依赖**——串行的"连环扣账"被一次求逆消灭。这就是 ③ 的意义：**(I+L)⁻¹ 是结算矩阵，把互相牵连的天真写入一次性换算成互不牵连的真实写入。**

**5. 为什么是下三角、对角线为什么是 1**

债只从过去流向未来（token i 只欠 j<i 的），所以干扰矩阵严格下三角——"未来"格子为零。对角线的 1 是每个方程里自己的系数（自己的未知数在自己方程里的系数就是 1）。

**6. 真实情况的三处推广**

1. 干扰量不是 1，而是 β_i·(k_i·k_j)·衰减——**② 算出的矩阵就是这个"干扰系数表"**（代码变量名 A，即 L）；
2. 每个写入不是标量而是 128 维向量——方程组的 U 和 B 是矩阵，解法不变；
3. 右端多一项：块初记忆 S₀ 的贡献——同一个 (I+L)⁻¹ 要乘两个右端项，所以 ③ 算出"逆"本身（供 ④ 乘两次：u 和 w），而不是只解一次方程。

**7. 求逆的过程：前代换 + 批量**

I+L 是单位下三角，不用通用求逆：从第 1 行顺序解到最后一行，第 i 行只用已解出的行——这就是 ③a 的 15 步行循环（16×16 时）。2048 个这样的小逆互相独立，一个核打包 38 个同时解（每步向量操作的宽度 = 38 块 × 256 元素）；64×64 的大逆由 4 个 16×16 小逆经 Schur 补拼接（③b，三次小矩阵乘、无串行）。串行深度的账：原始递推 T 步（8K → 8192 步）→ 块内行递推 15 步（常数，与 T 无关）→ Schur 合并无串行。
...
约定：prefill 主路径为主线；块 = 64 token；TP=2 每卡 16 个 v 头；C = Cube（AIC），V = Vector，MTE = 搬运引擎。

**第一段：公共前段（三路共用，只算一次）**

| 动作（单元） | 在算什么 | 注释 |
|---|---|---|
| 输入投影 ×3（C·GEMM） | mixed_qkv = hidden @ W_qkvz 等 | 权重常驻 GM，瓷砖流过 UB；按 TP 切列 |
| GQA 切分（V·Triton，可选） | 交错布局 → 按类型归位 | 一次读写重排，免多次拷贝 |
| 因果卷积（AscendC） | 与前 3 个位置加权混合 | **读写状态池 conv_state**（滑窗续接） |
| 门控（V·Triton） | g = −exp(A_log)·softplus(a+c)；β = sigmoid(b) | g 用 fp32，恒 ≤ 0 |
| 归一化＋切分（V·Triton） | q/k 归一化；按 spec/非 spec 索引拆 batch | index_select 是拷贝 |
| ① 前缀和（V·Triton） | gcum[i] = g[1]+…+g[i]（块内） | 任意两点衰减 = 查表相减 |
| ② 相似度矩阵（C+V） | C：L = k·kᵀ → V：×β、exp、下三角掩码 | 64×64/块·头 fp32；★1 |
| ③a 小块求逆（V） | 38 个 16×16 对角块行递推（15 步批量摊薄） | ★2 单位阵查表 |
| ③b 分块合并（C+V） | C：三次 Schur 补小乘 → V：拼装 | 16×16 小逆 → 64×64 的 (I+L)⁻¹ |
| ④ WY 重构（C+V） | V：预缩放 → C：u=(I+L)⁻¹(βV)、w=(I+L)⁻¹(βγK) | 产出"写什么"（u）和"扣多少旧账"（w） |
| ⊘ 布局转置（V） | k/w/u/g/q 转 ⑤⑥ 布局 | **纯搬运，5 张量各读写一遍** |

**第二段：prefill 逐块状态递推 ⑤（AscendC 混合核 MIX_AIC_1_2，每块循环四拍）**

| 动作（单元） | 在算什么 | 注释 |
|---|---|---|
| C1（Cube） | v_work = w·S_块初 | **算"回声有多大"**——u 里有多少是块初记忆的回响 |
| V1（AIV） | v_update = (u − v_work) ⊙ 衰减；k 乘衰减 | **扣回声＋按时间衰减** |
| C2（Cube） | h_work = kᵀ·v_update | **64 个写入一次矩阵乘全加上**——③④ 的并行累加在此兑现 |
| V2（AIV） | S_块末 = 衰减·S_块初 ＋ h_work；写快照 h、v_new | 末块把 S_final 写回**状态池**（转置+cast） |
| ⊘ 中转 | 四拍之间的 v_work/v_update/h_work 走 **GM workspace** | 跨核不共享 UB 的代价——账本外隐性流量 |

**第三段：prefill 输出 ⑥（AscendC 混合核，每块循环五拍）**

| 动作（单元） | 在算什么 | 注释 |
|---|---|---|
| C1（Cube） | attn = q·kᵀ（64×64） | 块内两两注意力分 |
| V1（AIV） | 因果掩码＋衰减 exp | 只允许看前面 |
| C2（Cube） | h_part = q·h_快照 | **块间读出**：从块前记忆取答案 |
| C3（Cube） | v_part = attn·v_new | **块内读出**：对本块净写入加权 |
| V2（AIV） | o = scale·(h_part ＋ v_part) | 两项相加，除以 √Dk |
| ⊘ 中转 | attn/aftermask/h_part/v_part 走 GM workspace | attn 一份 64×64 走四趟显存 |

**第四段：合并与输出（三路汇合）**

| 动作（单元） | 在算什么 | 注释 |
|---|---|---|
| 合并（V） | index_copy_ 按原 token 顺序写回 | 调用方感知不到分过路 |
| 输出门控（V） | o ⊙ z → RMSNorm | z 是投影切出的输出门 |
| 输出投影（C·GEMM） | out = ·W_out | 交给下一层 |

**decode/投机路径**（每步 1~k+1 token，AscendC 递推算子）：每 token 一圈 C/V 交替——检索（k·S）→ 扣差额（v−·）→ 衰减（×γ）→ 写入（S 累加）→ 读出（q·S），直接在状态池槽位原地更新；投机按采纳数回卷。

#### 三方时间分摊：bound 在哪

| 资源 | 每层每 8K prefill 总量 | 有效吞吐假设 | 时间 |
|---|---|---|---|
| **GM 搬运** | ~1.0 GB（账面 7.9 KB/token·头，**未计 ⑤⑥ 混合核内部 workspace 中转 +3.5~6.5 KB/token·头**） | 300 GB/s | **3.4~6 ms** |
| **Cube** | ~22 GFLOP（12 MFLOP/token·头 × 40% 效率） | 300 TFLOPS | **~0.2 ms** |
| **Vector** | ~3.5 亿元素操作（exp 按 8 倍折算） | ~11 Tops/s | **~0.03-0.1 ms** |
| Scalar/同步 | 8 次核启动 + ⑤⑥ 跨核 flag 互锁 | — | 0.1-0.3 ms |

**结论：GM 搬运 bound，量级上碾压其它三方**（搬运是 Cube 的 ~20 倍、Vector 的 ~100 倍）。三点佐证：单块内 Cube/Vector 耗时百纳秒级、MTE 搬运微秒级，差一个数量级；⑤⑥ 是混合核（MIX_AIC_1_2），Cube 阶段与 Vector 阶段的接力必经 GM workspace（跨核不共享 UB），核内有隐性搬运；⑥ 的 attn 一份 64×64 走四趟显存。Scalar 本身开销小，但 ⑤⑥ 的 flag 互锁和小 batch 下的核启动占比值得在 profiling 里关注。注意 ⑤⑥ 的隐性 workspace 流量意味着**真实总流量可能是 11-14 KB/token·头而非账面 7.9**——bound 的程度比账面更深，也因此下面流量类优化的收益可能取上限。
...
| D | ⑤⑥ 全融合 | h/v_new 落盘再读回 | 状态驻片上缓存逐块推进，只剩输入输出的必要流量（~2.6KB/token·头） | 流量降 2/3，端到端 20-30% | roofline 下限；⑤⑥ 混合核内部的 workspace 中转（见 9.1 执行表）使依据更硬；并行度要用 头×序列段 补偿，工程量最大 |
