...
- 怎么做：对每个 (块, 头) 算一个 64×64：
  ...
  ```
  A[i,j] = β[i] · (k_i·k_j) · exp(gcum[i] − gcum[j])    （只留 i > j）
  ```
...
- 得到什么：**债务矩阵 L**（代码变量名还叫 A）：L[i,j] = token i 检索时能碰到多少 token j 已写入的量。被扣掉的对角线项没有丢，它会变成 ③ 里 (I+L) 的那个 1。可以把它理解为 Softmax 注意力那张 T×T 大表压到 64×64 的版本。
